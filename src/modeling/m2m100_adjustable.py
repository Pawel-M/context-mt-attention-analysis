...
class AdjustableM2M100Attention(nn.Module):
...
class AdjustableM2M100EncoderLayer(M2M100EncoderLayer):
    def __init__(self, config: M2M100Config):
        nn.Module.__init__(self)
        self.embed_dim = config.d_model
        self.self_attn = AdjustableM2M100Attention(
...
class AdjustableM2M100DecoderLayer(M2M100DecoderLayer):
    def __init__(self, config: M2M100Config):
        nn.Module.__init__(self)
        self.embed_dim = config.d_model
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = AdjustableM2M100Attention(
            self.embed_dim,
...
class AdjustableM2M100Encoder(M2M100Encoder):
    """
    ...
    def __init__(self, config: M2M100Config, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([AdjustableM2M100EncoderLayer(config) for _ in range(config.encoder_layers)])
...
class AdjustableM2M100Decoder(M2M100Decoder):
    """
    ...
    def __init__(self, config: M2M100Config, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([AdjustableM2M100DecoderLayer(config) for _ in range(config.decoder_layers)])
        self.layer_norm = nn.LayerNorm(config.d_model)
    ...
    def forward(
        ...
        all_cross_pre_attentions = () if output_attentions else None
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if use_cache:
                next_decoder_cache += (layer_outputs[5 if output_attentions else 1],)
...
class AdjustableM2M100Model(M2M100Model):
    ...
    def __init__(self, config: M2M100Config):
        ...
        self.encoder = AdjustableM2M100Encoder(config, self.shared)
        self.decoder = AdjustableM2M100Decoder(config, self.shared)
    ...
    def forward(
            ...
            output_hidden_states: Optional[bool] = None,
            return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], AdjustableSeq2SeqModelOutput]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
...
class AdjustableM2M100ForConditionalGeneration(M2M100ForConditionalGeneration):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"encoder.version",
        r"decoder.version",
    ...
    def __init__(self, config: M2M100Config):
        M2M100PreTrainedModel.__init__(self, config)
        self.model = AdjustableM2M100Model(config)
    ...
    def forward(
            ...
            tgt_phrase_idx=None,
    ) -> Union[Tuple[torch.Tensor], AdjustableSeq2SeqLMOutput]:
        r"""
